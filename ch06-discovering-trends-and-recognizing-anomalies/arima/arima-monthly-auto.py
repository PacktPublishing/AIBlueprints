...
stepwise_model = auto_arima(series.ix['2015-01-01':'2016-12-31'],
                            start_p=1, start_q=1,
...
stepwise_model.fit(series.ix['2015-01-01':'2016-12-31'])
predictions = stepwise_model.predict(n_periods=len(series.ix['2017-01-01':]))
print(predictions)
print(series.ix['2017-01-01':][:len(predictions)])
print("Mean squared error: %.2f"
      % mean_squared_error(series.ix['2017-01-01':][:len(predictions)], predictions))

series.ix['2015-01-01':].plot(color='gray', linewidth=1)
...
plt.savefig('arima-monthly-auto-predictions.png', dpi=300, bbox_inches='tight', pad_inches=0)
