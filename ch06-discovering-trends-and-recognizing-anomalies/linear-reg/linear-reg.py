...
msgs_daily_cnts['date_delta'] = \
    (msgs_daily_cnts.index - msgs_daily_cnts.index.min()) / \
    np.timedelta64(1,'D')
msgs_daily_cnts.sort_values(by=['date_delta'])
print(msgs_daily_cnts)
print(len(msgs_daily_cnts.index))

train = msgs_daily_cnts[:-1000]
train_X = train['date_delta'].values.reshape((-1,1))
train_y = train['Message-ID']

test = msgs_daily_cnts[-1000:]
...
plt.savefig('linear-reg.png', dpi=300, bbox_inches='tight', pad_inches=0)
